...
We are building a LLM inference server that continously batches at the token level. At each token generation step, we check if any of the sequences have completed generation. If so, we insert a new sequence in its place. This approach allows us to achieve higher GPU utilization than static batching. We pull our messagesfrom Kafka

Next is an article on continuous batching

### Continuous Batching Reference

Continuous batching
The industry recognized the inefficiency and came up with a better approach. Orca: A Distributed Serving System for Transformer-Based Generative Models is a paper presented in OSDI ’22 which is the first to our knowledge to tackle this problem.
...
Here is a simple working example of how to generate a batch with qwen2.5
...
### Current Qwen Architecture

Here is the current Qwen2.5 architecture in huggingface
...
Ourt current goal is to make the server work with Qwen2.5.